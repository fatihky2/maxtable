...
BUF *
bufsearch(int blkno, int sstabno, int tabid)
{
	BUF	*bufptr;		
	BUF	**hashptr;		
	...
	{
		if (   (bufptr->bblkno == blkno) 
		    && (bufptr->bsstabid == sstabno)
		    && (bufptr->btabid = tabid))
		{
			V_SPINLOCK(BUF_SPIN);
			return (bufptr);
...
buffree(BUF *bp)
{
	...
	SSTABLE_STATE(bp) &= ~(BUF_NOTHASHED | BUF_DESTROY);
	return;
}
